...
## Event deck and timing

- Reveal and apply exactly 1 Event card every 2 rounds.
- Global modifier events last exactly 2 rounds:
  - Weekend Service: Limited/Select Bus cards are blocked.
  - Fare Hike: Bus->Rush trade requires Bus value 4.
  - Late Night Service: Express Bus plays are blocked.
  - Bridge Reconstruction: Transit perks are inactive while modifier lasts.
- One-time events:
  - Extra Service: each player draws 2 Bus cards.
  - Budget Cuts: each player discards 2 Bus cards.
  - No Smoking: each player loses 1 point from a chosen borough.
  - Borough parties: players holding a matching borough Bus card gain 1 point in that borough.
