...
export const purchaseTransitPerk = (state: GameState, input: PurchaseTransitPerkInput): GameState => {
  ...
  const selectedCards = currentPlayer.busHand.filter((card) => input.busCardIds.includes(card.id));
  const totalValue = selectedCards.reduce((sum, card) => sum + card.routeValue, 0);

  if (selectedCards.length === 0 || selectedCards.length !== input.busCardIds.length || totalValue !== TRANSIT_PERK_COST) {
  ...
  return {
    ...
    eventLog: [
      ...state.eventLog,
      `${currentPlayer.name} ${currentPlayer.activePerk ? "switched" : "purchased"} Transit Perk: ${input.perk.name}.`,
    ],
  };
};
