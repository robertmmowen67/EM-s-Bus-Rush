...
export const tradeRush = (state: GameState, input: RushTradeInput): GameState => {
  ...
  if (input.busCardIdsForRush) {
    ...
    const requiredValue = isGlobalModifierActive(state, "fare_hike") ? 4 : 3;
    if (selected.length === 0 || total !== requiredValue || selected.length !== input.busCardIdsForRush.length) {
      throw new Error(`Bus-to-Rush trade requires Bus cards totaling exactly ${requiredValue} value.`);
    }
...
export const endTurn = (state: GameState): GameState => {
  ...
  return applyEventAtRoundBoundary(nextState);
};
