...
describe("Transit perk objective scoring", () => {
  test("Express Rider grants +1 for Express Bus plays", () => {
    const state = {
      ...baseState([busCard("exp", "Manhattan", ["express"])]),
      currentBorough: "Queens" as const,
      players: baseState([busCard("exp", "Manhattan", ["express"])]).players.map((player, index) =>
        index === 0 ? { ...player, activePerk: perkCard("perk-exp", "express_rider") } : player,
      ),
    };
  ...
  test("Queens Bus Redesign grants +1 for Queens scoring", () => {
    const state = {
      ...baseState([busCard("q-perk", "Queens")]),
      players: baseState([busCard("q-perk", "Queens")]).players.map((player, index) =>
        index === 0 ? { ...player, activePerk: perkCard("perk-q", "queens_bus_redesign") } : player,
      ),
    };

    const next = playBusCard(state, { playerId: "p1", cardId: "q-perk" });
    expect(next.players[0].totalScore).toBe(2);
