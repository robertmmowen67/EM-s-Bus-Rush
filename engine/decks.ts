...
export const createDeckState = <TCard extends GameCard>(
  ...
  drawPile: shuffleWithSeed(cards, seed),
  discardPile: [],
});
...
const maybeReshuffleDiscardIntoDrawPile = <TCard extends GameCard>(
  ...
  return {
    reshuffled: true,
    deck: {
      drawPile: shuffleWithSeed(deck.discardPile, seed),
      discardPile: [],
    },
  };
};

export const drawCard = <TCard extends GameCard>(
  deck: DeckState<TCard>,
  seed: number,
): DrawResult<TCard> => {
...
export const drawCards = <TCard extends GameCard>(
  ...
  if (count <= 0) {
    return { deck, cards: [], reshuffled: false };
  }
...
export const discardCard = <TCard extends GameCard>(
  deck: DeckState<TCard>,
  card: TCard,
): DeckState<TCard> => ({
  ...deck,
...
export const discardCards = <TCard extends GameCard>(
  deck: DeckState<TCard>,
  cards: readonly TCard[],
): DeckState<TCard> => ({
  ...deck,
...
export const createBusDeck = (cards: readonly BusCard[], seed: number): DeckState<BusCard> =>
  createDeckState(cards, seed);

export const createRushDeck = (
  cards: readonly RushCard[],
  seed: number,
