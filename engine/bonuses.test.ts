...
describe("bonus scoring and win checks", () => {
  test("recalculateTotalScores includes both bonus awards", () => {
    const next = recalculateTotalScores(createState());

    expect(next.players[0].totalScore).toBe(17);
    expect(next.players[1].totalScore).toBe(5);
  });

  test("checkWin requires 14+ total score and at least one point in each borough", () => {
    const scored = recalculateTotalScores(createState());
    expect(checkWin(scored).winnerIds).toEqual(["p1"]);

    const noStatenIslandPoint = recalculateTotalScores({
      ...createState(),
      players: createState().players.map((player, index) =>
        index === 0
          ? { ...player, scoreByBorough: { ...player.scoreByBorough, StatenIsland: 0 } }
          : player,
      ),
    });

    expect(checkWin(noStatenIslandPoint).winnerIds).toEqual([]);
  });
});
