...
describe("Transit Perk purchasing and switching", () => {
  test("only the active player can purchase a Transit Perk", () => {
  ...
  test("requires Bus cards totaling exactly 3 value to purchase", () => {
    const state = createState([], [busCard("b1", "Queens", 1), busCard("b2", "Queens", 1)]);
  ...
  test("switching perks also costs Bus value 3 and keeps one active perk", () => {
    ...
    const firstPurchase = purchaseTransitPerk(state, {
      playerId: "p1",
      perk: perkCard("perk-1", "express_rider"),
      busCardIds: ["b1", "b2"],
    });

    expect(firstPurchase.players[0].activePerk?.effectKey).toBe("express_rider");

    const switched = purchaseTransitPerk(firstPurchase, {
      playerId: "p1",
    ...
    expect(switched.players[0].busHand).toHaveLength(0);
  });
});
