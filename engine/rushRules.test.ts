...
describe("Transit Perk purchases", () => {
  ...
  test("requires a valid Perk card", () => {
    const state = createState([], [busCard("b1", "Queens", 1), busCard("b2", "Queens", 2)]);

    expect(() =>
      purchaseTransitPerk(state, {
        playerId: "p1",
        perk: { id: "not-perk", name: "x", type: "rush", effectKey: "x" } as unknown as PerkCard,
        busCardIds: ["b1", "b2"],
      }),
    ).toThrow("Transit Perk purchase requires a valid Perk card.");
  });
  ...
  test("requires value 3 and switching also costs 3", () => {
    ...
    expect(purchased.busDeck.discardPile.map((card) => card.id)).toEqual(["b2", "b4"]);
